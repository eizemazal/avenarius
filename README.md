...
Скачать через [Google Play](https://play.google.com/store/apps/details?id=com.avenarius.app&hl=ru) - похоже сейчас Авенариус единственный клиент для Макса в Google Play.
...
В Авенариусе реализованы чаты, участие в группах, создание и администрирование групп, отправка и просмотр фото и видео, отправка и скачивание файлов, реакции, просмотр профилей пользователей, редактирование своего профиля, поиск по контактам, QR-коды для входа в веб-версию. Вход по СМС, двойная авторизация поддерживаются.

Поддерживаются звонки и видеозвонки (кроме групповых).

Пока не реализованы, но возможно будут реализованы групповые звонки, интеграция с Госуслугами, Сферум, темы, папки, избранное ~~и всесторонняя слежка~~.


**За что банят аккаунты в Максе?**

Меня банили за то, что я последовательно проверяю большое число телефонов на предмет их присутствия в MAX. Нативный клиент рассчитывает, что вся ваша адресная книга будет слита на сервер, и операция поиска человека по номеру телефона не очень-то и нужна. Авенариус не сливает адресную книгу, поэтому, когда вы хотите написать кому-то новому, кто не в ваших контактах на сервере, приходится сначала проверить, а есть ли этот телефон в Максе. Если сделать слишком много таких запросов (скажем, чтобы создать группу из новых телефонов) - можно схлопотать бан (я получил, сделав последовательно штук 20 запросов за несколько минут).
Поэтому скорость таких запросов в клиенте ограничена для безопасности вашего аккаунта. В списке контактов в приложении тоже нет контактов из адресной книги по той же причине.
